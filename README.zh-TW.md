...
**專案核心目標：**
目前官方的 Google Photos 並不支援以「資料夾」或「手機相簿」為單位進行上傳，導致所有照片備份後全混在一起，失去原有的分類。本專案致力於解決這個痛點，並提供兩種解決方案：

1. **桌面端應用程式 (Desktop App)**：透過電腦選取包含照片的總資料夾，程式會自動將照片上傳至 Google Photos，並在雲端建立同名的相簿，完美還原電腦上的資料夾分類結構。
2. **行動端應用程式 (Mobile App)**：直接讀取 Android 手機上的本地相簿（支援還原各家 Android 手機廠商的相簿分類），將照片自動備份並歸納到 Google Photos 對應的相簿當中。
